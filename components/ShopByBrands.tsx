...
import { GitCompareArrows, Headset, ShieldCheck, Truck, TruckIcon } from "lucide-react";

const extraData = [
  {
    title: "Free Delivery",
    description: "Free shipping over $100",
    icon: <TruckIcon size={45} />,
  },
  {
    title: "Free Return",
    description: "Free shipping over $100",
  ...
  return (
    <div className='mb-10 lg:mb-20 bg-shop_light_bg p-5 lg:p-7 rounded-md'>
      ...
      <div className='grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-2.5' >
        {brands?.map((brand)=> (
          <Link key={brand._id} href={`/brand/${brand?.slug?.current}`} className='bg-white w-34 h-24 flex items-center justify-center rounded-md overflow-hidden hover:shadow-lg shadow-shop_dark_green/20 hoverEffect'>
           {brand?.image && <Image src={urlFor(brand?.image).url()} alt='brandImage'width={250} height={250} className='w-32 h-20 object-contain'/>}
          </Link>
        ))}
      </div>

      <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-16 p-2 shadow-shop_light_green/20 py-5'>
