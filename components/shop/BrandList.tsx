  ...
  return (
    <div className='w-full bg-white p-5'>
      ...
      <RadioGroup value={selectedBrand || ""} className='mt-2 space-y-1'>
        {brands?.map((brand)=>(
          <div key={brand?._id} onClick={() => setSelectedBrand(brand?.slug?.current as string)} className='flex items-center space-x-2 hover:cursor-pointer'>
            <RadioGroupItem value={brand?.slug?.current as string} id={brand?.slug?.current} className='rounded-sm'/>
            <Label htmlFor={brand?.slug?.current} className={`${selectedBrand === brand?.slug?.current ? "font-semibold":"font-medium"}`}>{brand?.title}</Label>
          </div>
        ))}
        {selectedBrand && (
