...
const CategoryList = ({categories,setSelectedCategory,selectedCategory}:Props) => {
  return (
    <div className='w-full bg-white p-5'>
      ...
      <RadioGroup value={selectedCategory || ""} className='mt-3 space-y-1'>
        {categories?.map((category)=> (
          ...
          key={category?._id} className='flex items-center space-x-2 hover:cursor-pointer'>
            ...
            <Label htmlFor={category?.slug?.current} className={`${selectedCategory === category?.slug?.current ? "font-semibold text-shop_dark_green":"font-normal"}`}>{category?.title}</Label>
          </div>
        ))}
      </RadioGroup>
      {selectedCategory && (
