...
const PriceList = ({selectedPrice,setSelectedPrice}:Props) => {
  return (
    <div className='w-full bg-white p-5'>
      ...
      <RadioGroup className='mt-2 space-y-1' value={selectedPrice || ""}>
        {priceArray?.map((price,index)=>(
          <div key={index} onClick={() => setSelectedPrice(price?.value)} className='flex items-center space-x-2 hover:cursor-pointer'>
            ...
            <Label htmlFor={priceArray[0]?.value} className={`${selectedPrice === price?.value ?"font-semibold text-shop_dark_green":"font-normal"}`}>
              {price?.title}
            </Label>
          </div>
        ))}
      ...
      {selectedPrice && (
        <button onClick={()=> setSelectedPrice(null)} className='text-sm font-medium mt-2 underline underline-offset-2 decoration-1'>Reset Selection</button>
      )}
    </div>
  )
}
